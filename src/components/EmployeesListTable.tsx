        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <input
              ...
              className="w-72 pl-9 pr-3 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#091e65]"
            />
          </div>
          <button
            onClick={onSearch}
      ...
      <div className="overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600">Profile</th>
              ...
              <th className="px-4 py-3 text-right text-xs font-semibold text-gray-600">Joined</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {loading && (
              <tr>
                <td colSpan={6} className="px-4 py-6 text-center text-gray-500 text-sm">Loading...</td>
              </tr>
            )}
            {!loading && rows.length === 0 && (
              <tr>
                <td colSpan={6} className="px-4 py-6 text-center text-gray-500 text-sm">No employees found</td>
              </tr>
            )}
            {!loading && rows.map((e) => (
              <tr key={e.employeeId} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-sm text-gray-800">{e.designation || '-'}</td>
                <td className="px-4 py-3 text-sm text-gray-800">{e.department || '-'}</td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3 text-right text-sm text-gray-600">{e.createdAt ? new Date(e.createdAt).toLocaleDateString() : '-'}</td>
              </tr>
            ))}
          </tbody>
        </table>
